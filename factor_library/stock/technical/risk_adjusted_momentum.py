...
@register_factor
class StockRiskAdjustedMomentum(Factor):
    """
    股票风险调整动量因子（复合因子）
    ...
    **核心特性**: 依赖其他因子，测试DAG调度
    ...
    依赖:
    - stock_momentum_return: 股票动量因子
    - stock_volatility: 股票波动率因子

    方向：数值越大，风险调整后收益越高
    """

    # 元数据
    name = "stock_risk_adjusted_momentum"
    input_type = "bar"
    max_lookback = 250
    ...
    # **核心**: 声明依赖
    dependencies = ["stock_momentum_return", "stock_volatility"]
    ...
    def compute(self, data: pd.DataFrame, deps: Optional[Dict[str, pd.Series]] = None) -> pd.Series:
        ...
        Args:
            ...
            deps: 依赖因子的计算结果
                - deps['stock_momentum_return']: 股票动量因子值
                - deps['stock_volatility']: 股票波动率因子值

        Returns:
            风险调整动量值
        """
        # 防御性检查
        if deps is None or 'stock_momentum_return' not in deps or 'stock_volatility' not in deps:
            raise ValueError("缺少依赖因子: stock_momentum_return, stock_volatility")

        momentum = deps['stock_momentum_return']
        vol = deps['stock_volatility']
...
__all__ = ['StockRiskAdjustedMomentum']
