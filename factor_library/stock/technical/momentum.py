...
@register_factor
class StockSimpleMomentum(Factor):
    """
    股票经典截面动量因子 (Simple Momentum)

    计算公式: 今日 hfq_close / N日前的 hfq_close - 1
    方向：一般数值越大，动量越强；但在某些市场（如A股短线）可能是反转效应
    """

    name = "stock_simple_momentum"
    input_type = "bar"
    max_lookback = 250
...
@register_factor
class StockMomentumReturn(Factor):
    """
    股票动量收益率因子
...
if __name__ == "__main__":
    ...
    运行方式:
        python factor_library/stock/technical/momentum.py
    """
    import sys
    ...
    # 1. 测试因子创建
    factor = StockMomentumReturn(timeframe='1d', para={'window': 20})
    print(f"\n✅ 因子实例: {factor}")
    ...
    registered = FactorRegistry.get('stock_momentum_return')
