...
class PostProcessor:
    ...
    @staticmethod
    def standardize(
        series: pd.Series, method: Literal["zscore", "minmax"] = "zscore"
    ) -> pd.Series:
        """
        标准化
    ...
    @staticmethod
    def check_validity(
        series: pd.Series, max_nan_ratio: float = 0.1, max_inf_ratio: float = 0.01
    ) -> dict:
        """
        有效性检查
        ...
        if nan_ratio > max_nan_ratio:
            warnings.warn(f"NaN 比例过高: {nan_ratio:.2%} (阈值: {max_nan_ratio:.2%})")

        if inf_ratio > max_inf_ratio:
            warnings.warn(f"Inf 比例过高: {inf_ratio:.2%} (阈值: {max_inf_ratio:.2%})")
    ...
    @staticmethod
    def fillna(
        series: pd.Series, method: Literal["ffill", "bfill", "mean", "zero"] = "ffill"
    ) -> pd.Series:
        """
        填充NaN值
