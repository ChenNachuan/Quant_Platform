"""
Random Forest Strategy Runner.

运行 Random Forest ML 因子策略的完整流程。
"""
import sys
...
def main():
    ...
    all_predictions.to_parquet(output_dir / 'rf_predictions.parquet', index=False)

    # 4. IC 检验
    ic_series = ICEvaluator.calculate_ic(all_predictions)
    ic_series.to_csv(output_dir / 'rf_ic_series.csv')
    ...
    # 保存回测结果
    performance_df.to_parquet(output_dir / 'rf_performance.parquet')
...
def plot_results(performance_df, ic_series, data_config):
    ...
    plt.title('Random Forest 策略表现', fontsize=18, fontweight='bold')
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(output_dir / 'rf_cumulative_returns.png', dpi=150)
    plt.show()
    ...
    plt.title('Random Forest Rolling IC', fontsize=15)
    plt.ylabel('IC Value', fontsize=12)
    plt.xlabel('Date', fontsize=12)
    plt.legend(loc='best')
    plt.grid(True, linestyle='--', alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_dir / 'rf_rolling_ic.png', dpi=150)
    plt.show()
