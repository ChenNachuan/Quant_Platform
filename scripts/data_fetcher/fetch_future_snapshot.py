...
注意：涉及夜盘快照，20:00 作为夜盘分割时间点

docker run --rm \
  -v ./data_lake/future:/data/future \
  --env-file ~/AmazingData/docker/.env \
  --entrypoint python3 \
  docker-amazingdata \
  /app/scripts/data_fetcher/fetch_future_snapshot.py
"""
...
# 期货快照数据目录
FUTURE_DATA_DIR = Path("/data/future/snapshot/1d")
        ...
        for y, m, begin, end in month_range(START_YEAR, START_MONTH, 2026, 6):
            ...
            try:
                ...
                if frames:
                    ...
                    year_dir = FUTURE_DATA_DIR / f"year={y}"
